...
task :parallel => files do |t|
	...
	sh "bsub -P #{ENV['LSF_PROJECT']} -J parallelbc rake -f #{__FILE__} barcodes #{cfname}"
	end
end
...
def summtable(filelist,outfile,filehash,fieldsep="_",sep="\t")
...
# Add zero array to any barcode that wasn't seen in this file
	...
	entry = filehash[f.sub(/\.(\w+).map/,'.fastq')]
...
simss = files.map{|a| a.sub('.fastq','.sims.map')}
wests = files.map{|a| a.sub('.fastq','.west.map')}
summtable(simss,"sims-summary.tsv",fileh,fieldsep,sep)
summtable(wests,"west-summary.tsv",fileh,fieldsep,sep)
